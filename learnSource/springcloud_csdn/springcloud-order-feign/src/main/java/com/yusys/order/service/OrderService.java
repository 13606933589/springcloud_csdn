...
import com.yusys.order.entity.OrderDetail;
...
public class OrderService {
    ...
    public Order queryOrderByIdFeign(String orderId) {
        Order order = ORDER_DATA.get(orderId);
        if (null == order) {
            return null;
        }
        ...
        for (OrderDetail orderDetail : orderDetails) {
            // 通过商品微服务查询商品详细数据
            Item item = this.itemService.queryItemByIdFegin(orderDetail.getItem()
                    .getId());
            if (null == item) {
                continue;
            }
